...
bool UpdateOptionsFromSystemProperties(Options& options) {
    ...
    Log::Write(Log::Level::Info, value);
...
void android_main(struct android_app* app) {
    try {
        ...
        Log::Write(Log::Level::Info, "===> start <===");

        app->userData = &appState;
        app->onAppCmd = app_handle_cmd;

        std::shared_ptr<Options> options = std::make_shared<Options>();
        // set opengles as render api
        ...
        // std::make_shared<AndroidPlatformPlugin>(options, data);
...
//            Log::Write(Log::Level::Info, "Main xrInitializeLoaderKHR");
//        }

        program->CreateInstance();
        program->InitializeSystem(); // =>  m_graphicsPlugin->InitializeDevice(m_instance, m_systemId); 初始话GL环境，初始化要绘制的立方体
